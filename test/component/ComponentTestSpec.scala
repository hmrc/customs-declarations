...
trait ComponentTestSpec extends FeatureSpec with GivenWhenThen with GuiceOneAppPerSuite
  ...
  override implicit lazy val app: Application = new GuiceApplicationBuilder().configure(Map(
    ...
    "microservice.services.upscan.host" -> ExternalServicesConfig.Host,
    "microservice.services.upscan.port" -> ExternalServicesConfig.Port,
    "microservice.services.upscan.context" -> CustomsDeclarationsExternalServicesConfig.UpscanInitiateContext,
    "microservice.services.upscan.bearer-token" -> ExternalServicesConfig.AuthToken,
