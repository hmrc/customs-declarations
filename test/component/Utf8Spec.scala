...
class Utf8Spec extends ComponentTestSpec with AuditService with ExpectedTestResponses
  ...
  val ValidRawXmlByte = ByteString.fromArray(Files.readAllBytes(Paths.get("target/scala-3.3.3/test-classes/raw/valid_xml.raw")))
  val InvalidRawXmlByte = ByteString.fromArray(Files.readAllBytes(Paths.get("target/scala-3.3.3/test-classes/raw/invalid_xml.raw")))
