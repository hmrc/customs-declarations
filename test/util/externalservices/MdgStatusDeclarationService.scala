...
trait MdgStatusDeclarationService extends WireMockRunner {
  ...
  def startMdgStatusV3Service(status: Int = OK, body: NodeSeq = StatusTestXMLData.generateDeclarationStatusResponse(acceptanceOrCreationDate = acceptanceDateVal)): Unit = startService(status, v3URL, body)
