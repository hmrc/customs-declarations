...
trait MdgStatusDeclarationService extends WireMockRunner {
  ...
  def startMdgStatusV2Service(status: Int = OK, body: NodeSeq = StatusTestXMLData.generateDeclarationStatusResponse(acceptanceDate = acceptanceDateVal)): Unit = startService(status, v2URL, body)

  def startMdgStatusV3Service(status: Int = OK, body: NodeSeq = StatusTestXMLData.generateDeclarationStatusResponse(acceptanceDate = acceptanceDateVal)): Unit = startService(status, v3URL, body)
